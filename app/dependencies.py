...
def get_user_repo(db: Session = Depends(get_db)):                       # UserRepository is a class to handle users storing in a data base, here with depends we instanciate the class session with Depends() which directly inject the parameter needed (here: db, using get_db)
